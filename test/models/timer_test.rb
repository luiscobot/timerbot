...
class TimerTest < ActiveSupport::TestCase
  ...
  test "both slugs are lowercase, legible and independent of each other" do
    10.times do
      ...
      assert_match(/\A[a-z0-9]{6}\z/, timer.slug)
      assert_match(/\A[a-z0-9]{6}\z/, timer.watch_slug)
      # The lookalikes of 1 and 0 the alphabet leaves out.
      assert_no_match(/[ilo]/, timer.slug + timer.watch_slug)
      assert_not_equal timer.slug, timer.watch_slug
    end
  end
  ...
  test "the watch slug is never minted equal to the control slug" do
    ...
    assert_equal "xyz789", timer.watch_slug
  ensure
  ...
  test "either slug is found whatever case it arrives in" do
    ...
    assert_equal timer, Timer.watched_by(timer.watch_slug.upcase)
  end

  # The watch slug can never find a timer to drive.
  test "neither slug answers for the other" do
    timer = timers(:one)

    assert_raises(ActiveRecord::RecordNotFound) { Timer.controlled_by(timer.watch_slug) }
    assert_raises(ActiveRecord::RecordNotFound) { Timer.watched_by(timer.slug) }
  end
  ...
  test "the finders fold back a timer that ran out unwatched" do
    ...
    travel_to Time.current do
      ...
      timer.reload.start
      travel 700.seconds
      assert Timer.watched_by(timer.watch_slug).idle?
    end
  end
  ...
  # An hour chosen is an hour already running: nothing is left to press.
  test "running_until leaves the timer running" do
    freeze_time do
      timer = Timer.running_until(10.minutes.from_now)
  ...
  # The whole second the ceiling added is given straight back by started_at,
  # so what is left is the gap itself and not the second above it.
  test "running_until counts whole seconds and starts the fraction ago" do
    freeze_time do
      deadline = Time.current + 600.5

      timer = Timer.running_until(deadline)

      assert_equal 601, timer.duration
      assert_equal deadline - 601, timer.started_at
      assert_in_delta 600.5, timer.remaining, 0.001
    end
  end

  test "running_until reaches zero at the instant it was given" do
    travel_to Time.current do
      deadline = Time.current + 600.5
      timer = Timer.running_until(deadline)

      travel 600.seconds
      assert_in_delta 0.5, timer.remaining, 0.001

      # The default travel_to drops the fraction, which is the whole point here.
  ...
  test "pause after running_until keeps the time left to the instant" do
    travel_to Time.current do
      timer = Timer.running_until(10.minutes.from_now)
  ...
  # Where a countdown ends is zero, not the duration it started from: everyone
  # watching is left looking at the clock that ran out.
  test "an expired running timer stops at zero when completed" do
    timer = timers(:one)
  ...
  test "complete stops an expired timer and is idempotent" do
    timer = timers(:one)
  ...
  # The zero a countdown ended on is not a duration to run again, and pressing
  # Reiniciar first is not worth asking for.
  test "start on a spent clock starts the duration over" do
    timer = timers(:one)
    ...
    travel_to Time.current do
      ...
      assert timer.running?
      assert_equal 600, timer.remaining_seconds
      assert_in_delta 600.0, timer.remaining, 1.0
    end
  end

  # Reiniciar is the other way out: the duration back, and nothing running.
  test "reset puts the duration back on a spent clock" do
    timer = timers(:one)
