...
class Timers::RunsControllerTest < ActionDispatch::IntegrationTest
  ...
  # A clock left on the zero it counted down to: Iniciar runs it again rather
  # than asking for Reiniciar first.
  test "creating it on a spent clock starts the duration over" do
    travel_to Time.current do
      post timer_run_path(timers(:one))
      travel 700.seconds
      post timer_run_path(timers(:one))
