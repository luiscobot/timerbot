...
class TimersControllerTest < ActionDispatch::IntegrationTest
  ...
  # A deadline is already a start: what comes out is a timer already falling,
  # so it lands on zero at the hour that was chosen.
  test "a deadline creates a timer already running" do
    freeze_time do
      assert_difference("Timer.count") do
        post timers_path, params: { deadline: iso(10.minutes.from_now) }
      end
  ...
  test "a deadline hands Proyectar both paths, already running" do
    freeze_time do
      post timers_path, params: { duration: 300, deadline: iso(10.minutes.from_now) },
                        as: :json
      ...
      assert_equal watch_path(timer.watch_slug), body["watch"]
      assert timer.running?
    end
  end

  # The form always sends both fields, and the hour is the one that was chosen.
  test "a deadline wins over the duration sent with it" do
    freeze_time do
      post timers_path, params: { duration: 125, deadline: iso(10.minutes.from_now) }

      assert_equal 600, Timer.last.duration
    end
  end

  test "a blank deadline takes the duration path and leaves the timer idle" do
    post timers_path, params: { duration: 125, deadline: "" }
  ...
  # start would begin the countdown from now, a fraction past the instant.
  test "start does not move a deadline" do
    travel_to Time.current do
      post timers_path, params: { deadline: iso(Time.current + 600.5), start: "Iniciar" }

      timer = Timer.last
      assert timer.running?
      assert_in_delta 600.5, timer.remaining, 0.001
    end
  end

  # The same screen the limit answers on, so the clock and the chosen hour
  # survive the refusal.
  test "a deadline that already passed is refused on the entry page" do
    passed = iso(1.minute.ago)

    assert_no_difference("Timer.count") do
      post timers_path, params: { duration: 125, deadline: passed }
    end
  ...
  test "a deadline past the ceiling is refused" do
    assert_no_difference("Timer.count") do
      post timers_path, params: { duration: 125, deadline: iso(90.minutes.from_now) }
    end
  ...
  # wants; Proyectar asks in JSON.
  test "a refusal answers Proyectar with the page" do
    post timers_path, params: { duration: 125, deadline: iso(1.minute.ago) },
                      as: :json
  ...
  test "create refuses a deadline it cannot read" do
    assert_no_difference("Timer.count") do
      post timers_path, params: { duration: 125, deadline: "mañana" }
  ...
  # A deadline mints a record like any other create.
  test "a deadline counts against the creation limit" do
    TimersController::CREATION_LIMIT.times { post timers_path, params: { duration: 300 } }
  ...
  # An hour is posted without a duration, and the refusal re-reads what it was
  # sent: reading it as required would answer 400 instead of the limit.
  test "a deadline with no duration still gets the limit's answer" do
    TimersController::CREATION_LIMIT.times { post timers_path, params: { duration: 300 } }
  ...
  test "the root offers the hour pill and the field behind it" do
    get root_path

    assert_select "label.control.deadline[title=?]", "Hasta las (H)"
    assert_select "label.control.deadline select[data-duration-target=time]"
    assert_select "label.control.deadline select[data-action*=?]", "duration#aim"
    assert_select "label.control.deadline select[data-action*=?]", "duration#restock"
    # The hours are instants, and the zone is the browser's, so the client
    # stocks the list. The server ships the blank and nothing else.
    assert_select "label.control.deadline select option", count: 1
    assert_select "label.control.deadline select option", text: "--:--"
    # Named, it would post a wall-clock hour the server cannot place in a zone.
    assert_select "label.control.deadline select[name]", count: 0
    assert_select "input[type=hidden][name=deadline][data-duration-target=deadline]"
  ...
  test "create hands back both paths as json" do
    ...
    assert_equal timer_path(timer), body["dashboard"]
    assert_equal watch_path(timer.watch_slug), body["watch"]
    assert_no_match timer.slug, body["watch"]
  end
  ...
  test "the entry page binds the shortcuts too" do
    ...
    assert_select ".control.watch[title=?]", "Proyectar (P)"
  ...
  # The watch page renders the same clock partial and must not learn the
  # control slug.
  ...
  test "showing a timer that ran out unwatched folds it back to idle at zero" do
    travel_to Time.current do
      timers(:one).start
      travel 700.seconds
  ...
  test "the watch slug cannot be used to control the timer" do
    timer = timers(:one)
    watching = timer.watch_slug

    post "/t/#{watching}/run"
    assert_response :not_found, "start answered to the watch slug"

    delete "/t/#{watching}/run"
    assert_response :not_found, "pause answered to the watch slug"

    post "/t/#{watching}/reset"
    assert_response :not_found, "reset answered to the watch slug"

    patch "/t/#{watching}", params: { duration: 90 }
    assert_response :not_found, "update answered to the watch slug"
  ...
  private
    # What Date#toISOString() sends: UTC, to the millisecond.
    def iso(time)
      time.utc.iso8601(3)
    end
end
