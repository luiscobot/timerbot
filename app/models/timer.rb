class Timer < ApplicationRecord
  ...
  # hand.
  normalizes :slug, :watch_slug, with: ->(slug) { slug.downcase }
  ...
  # Named for the column each kind of link may answer to, so the watch slug
  # can never find a timer to drive. Both hand back a settled timer: expiry
  # is folded in at lookup, so no caller has to remember to.
  def self.controlled_by(slug)
    find_by!(slug: slug).tap(&:complete)
  end

  def self.watched_by(watch_slug)
    find_by!(watch_slug: watch_slug).tap(&:complete)
  end
  ...
  # Born running, so it reaches zero at that instant. started_at goes back by
  # the fraction the ceiling added, which leaves remaining == deadline - now.
  # The range is refused before normalizes sees it: clamping would hand back a
  # countdown to some other moment instead of saying no.
  ...
  # A clock that ran out starts over: what it has left is the zero it stopped
  # on, and pressing Reiniciar first is not worth asking for.
  def start
    complete
    return if running?

    update!(status: :running, started_at: Time.current,
            remaining_seconds: remaining_seconds.zero? ? duration : remaining_seconds)
    broadcast_state
  end
  ...
  # client's countdown hits zero. The clock stops on the zero it counted down
  # to rather than springing back to the duration: that is where a countdown
  # ends, and it is the same clock everyone watching is left looking at.
  # Reiniciar puts the duration back, Iniciar starts it over. Idempotent.
  def complete
    return unless expired?

    update!(status: :idle, remaining_seconds: 0, started_at: nil)
    broadcast_state
  end
  ...
  private
    ...
    def generate_slugs
      ...
      self.watch_slug ||= unique_slug(:watch_slug)
    end

    # Skips duration-shaped candidates, so /t/12m34s is never a real page, and
    # the slug already minted, so the watch link can never drive.
